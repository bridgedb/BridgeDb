...
do
 ...
 then
  ...
  export species_name="$species_name"; var=`echo "ls -l /pub/plants/release-4/mysql/" | ncftp ${ftp_site} | perl -ane 'if ( $_ =~ /$Logged in to/ ) { $go=1; next } if ( $go ) { @a=split; $fileName=$a[ scalar( @a ) - 1]; $compareStr=$ENV{ species_name }."_core"; if ( $fileName =~/^$compareStr/) { print $fileName."\n"; } }'`; set -- $var; mysql_db_name=$1
  ...
  ncftpget ${ftp_site} ./${mysql_db_name} /pub/plants/release-4/mysql/${mysql_db_name}/*.txt.gz
  for q in ./${mysql_db_name}/*.txt.gz
  do
    gunzip $q
  done

  ncftpget ${ftp_site} ./ /pub/plants/release-4/mysql/${mysql_db_name}/${mysql_db_name}.sql.gz
