...
do
 ...
 then
  ...
  export species_name="$species_name"; var=`echo "ls -l /pub/release-${release}/bacteria/mysql/" | ncftp ${ftp_site} | perl -ane 'if ( $_ =~ /$Logged in to/ ) { $go=1; next } if ( $go ) { @a=split; $fileName=$a[ scalar( @a ) - 1]; $compareStr=$ENV{ species_name }."_core"; if ( $fileName =~/^$compareStr/) { print $fileName."\n"; } }'`; set -- $var; mysql_db_name=$1
  ...
  ncftpget ${ftp_site} ./${mysql_db_name} /pub/release-${release}/bacteria/mysql/${mysql_db_name}/*.txt.gz
  for q in ./${mysql_db_name}/*.txt.gz
  do
    gunzip $q
  done

  ncftpget ${ftp_site} ./ /pub/release-${release}/bacteria/mysql/${mysql_db_name}/${mysql_db_name}.sql.gz
...
##FUNCGEN
  export species_name="$species_name"; var=`echo "ls -l /pub/release-${release}/bacteria/mysql/" | ncftp ${ftp_site} | perl -ane 'if ( $_ =~ /$Logged in to/ ) { $go=1; next } if ( $go ) { @a=split; $fileName=$a[ scalar( @a ) - 1]; $compareStr=$ENV{ species_name }."_funcgen"; if ( $fileName =~/^$compareStr/) { print $fileName."\n"; } }'`; set -- $var; mysql_efg_name=$1
  ...
  # get raw tables from ftp site
  ncftpget ${ftp_site} ./${mysql_efg_name} /pub/release-${release}/bacteria/mysql/${mysql_efg_name}/*.txt.gz
  for q in ./${mysql_efg_name}/*.txt.gz
  do
    gunzip $q
  done

  ncftpget ${ftp_site} ./ /pub/release-${release}/bacteria/mysql/${mysql_efg_name}/${mysql_efg_name}.sql.gz
