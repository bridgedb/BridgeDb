...
do
 ...
 then
  ...
  export species_name="$species_name"; var=`echo "ls -l /pub/bacteria/release-1/mysql/" | ncftp ${ftp_site} | perl -ane 'if ( $_ =~ /$Logged in to/ ) { $go=1; next } if ( $go ) { @a=split; $fileName=$a[ scalar( @a ) - 1]; $compareStr=$ENV{ species_name }."_core"; if ( $fileName =~/^$compareStr/) { print $fileName."\n"; } }'`; set -- $var; mysql_db_name=$1
  ...
  ncftpget ${ftp_site} ./${mysql_db_name} /pub/bacteria/release-1/mysql/${mysql_db_name}/*.txt.table.gz
  for q in ./${mysql_db_name}/*.txt.table.gz
  do
    gunzip $q
  done

  ncftpget ${ftp_site} ./ /pub/bacteria/release-1/mysql/${mysql_db_name}/${mysql_db_name}.sql.gz
  ...
  (
    cd ./${mysql_db_name}; for table_name in *.txt.table
    do
      #import data
      cp $table_name /tmp/

      ${mysqlimport} -u genmapp -pfun4genmapp ${mysql_db_name_local} /tmp/${table_name}
  ...
  tar cvzf ./${mysql_db_name}.txt.table.tgz ./${mysql_db_name}/*
  rm -R ./${mysql_db_name}/
