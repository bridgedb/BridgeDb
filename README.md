...
Usage depends on which module you require. The examples below assumes artifact `org.bridgedb.bio` and version `3.1.3`:
...
```gradle
compile group: 'org.bridgedb', name: 'org.bridgedb.bio', version: '3.1.3'
```

For [Ivy](http://ant.apache.org/ivy/):

```xml
<dependency org="org.bridgedb" name="org.bridgedb.bio" rev="3.1.3"/>
```

For [Buildr](https://buildr.apache.org/):

```buildr
'org.bridgedb:org.bridgedb.bio:jar:3.1.3'
```
...
```shell
mvn versions:set -DnewVersion=3.1.3
mvn clean deploy
```
...
```shell
mvn versions:set -DnewVersion=3.1.4-SNAPSHOT
mvn clean deploy
```
