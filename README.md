...
Usage depends on which module you require. The examples below assumes artifact `org.bridgedb.bio` and version `3.0.31`:
...
```shell
mvn versions:set -DnewVersion=3.0.31
mvn clean deploy
```
...
```shell
mvn versions:set -DnewVersion=3.0.32-SNAPSHOT
mvn clean deploy
```
